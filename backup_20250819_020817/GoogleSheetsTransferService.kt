...
class GoogleSheetsTransferService @Inject constructor(
    ...
    /**
     * Convierte datos de AUSENCIAS-LOG a AbsenceRecord
     */
    private fun convertLogsToAbsenceRecords(data: List<List<Any>>): List<AbsenceRecord> {
        if (data.size <= 1) return emptyList() // Solo header o sin datos
        
        val absenceRecords = mutableListOf<AbsenceRecord>()
        val dateFormat = SimpleDateFormat(DATE_FORMAT, Locale.getDefault())
        ...
        for (i in 1 until data.size) {
            ...
            if (row.size >= 9) { // Mínimo de columnas necesarias (A-I)
                try {
                    ...
                    var employeeId = row[1].toString() // Columna B: USUARIO
                    var employeeName = row[2].toString() // Columna C: NOMBRE COMPLETO
                    
                    // NORMALIZACIÓN DEL employeeId EN EL ORIGEN
                    // Si employeeId (de USUARIO) es numérico y employeeName (de NOMBRE COMPLETO) parece un app ID,
                    // entonces usar employeeName como employeeId para consistencia
                    if (employeeId.matches(Regex("\\d+")) && employeeName.matches(Regex("[a-zA-Z]+\\.[a-zA-Z]+"))) {
                        Log.d(TAG, "Normalizando employeeId: '$employeeId' (numérico) y employeeName: '$employeeName' (app ID). Usando employeeName como employeeId.")
                        employeeId = employeeName // Usar el app ID de employeeName
                        // employeeName se actualizará más tarde en FragmentAusenciasCuadrante si sigue siendo el app ID
                    }
                    // FIN NORMALIZACIÓN
                    ...
                    Log.d(TAG, "Datos extraídos - ID: $employeeId, Nombre: $employeeName, Tipo: $typeStr, Inicio: $startDateStr, Fin: $endDateStr")
                    ...
                    if (startDate != null && endDate != null) {
                        ...
                        val absenceRecord = AbsenceRecord(
                            employeeId = employeeId,
                            employeeName = employeeName, // Usar el nombre completo real
                            employeeEmail = "$employeeId@empresa.com",
                            startDate = startDate,
                            endDate = endDate,
                        ...
                        Log.d(TAG, "Registro agregado: ${absenceRecord.employeeName} - ${absenceRecord.absenceType.displayName}")
                    } else {
        ...
        Log.d(TAG, "Registros convertidos desde AUSENCIAS-LOG: ${absenceRecords.size}")
