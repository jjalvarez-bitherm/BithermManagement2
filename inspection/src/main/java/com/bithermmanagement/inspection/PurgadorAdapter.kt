...
class PurgadorAdapter : RecyclerView.Adapter<PurgadorAdapter.PurgadorViewHolder>() {
    ...
    private var onItemClickListener: ((EquipoView) -> Unit)? = null
    ...
    fun setOnItemClickListener(listener: (EquipoView) -> Unit) {
    ...
    inner class PurgadorViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        init {
            itemView.setOnClickListener {
                ...
                if (position != RecyclerView.NO_POSITION) {
                    onItemClickListener?.invoke(purgadores[position])
                }
            }
        }
